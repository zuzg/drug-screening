...
def histogram_control_value(
    df: pd.DataFrame, feature: str, control_pos: str, control_neg: str
) -> sns.displot:
    """
    ...
    histogram.vlines(
        df[control_neg], 0, 1300, colors="red", linestyles="dotted", label="negative"
    )
    histogram.vlines(
        df[control_pos], 0, 1300, colors="green", linestyles="dotted", label="positive"
    )
...
def plot_projections(X_umap: np.ndarray, df: pd.DataFrame) -> None:
    ...
    for col, i, ax in zip(df.columns[1:], range(9), axs.ravel()):
        ax.scatter(X_umap[:, 0], X_umap[:, 1], c=df[col], cmap="coolwarm", s=5)
        ax.set_title(col)
...
def projection_2d_add_controls(
    fig: px.scatter,
