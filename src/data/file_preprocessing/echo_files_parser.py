...
class EchoFilesParser:
    ...
    def parse_files(self) -> EchoFilesParser:
        ...
        for filename in self.echo_files: 
            ...
            self.exceptions_df = exceptions_df if self.exceptions_df.empty else pd.concat([self.exceptions_df, exceptions_df], ignore_index=True)
            self.echo_df = echo_df if self.echo_df.empty else pd.concat([self.echo_df, echo_df], ignore_index=True)
    ...
    def link_bmg_files(self, 
        ...
        """
        echo_bmg_linked_df = pd.DataFrame()
        ...
        for bmg_file in bmg_files:
            ...
            echo_bmg_linked_df = self.echo_df.merge(bmg_df, 
                                              left_on=echo_keys, 
                                              right_on=bmg_keys) if echo_bmg_linked_df.empty else pd.concat([echo_bmg_linked_df, 
                                                                                                            self.echo_df.merge(bmg_df, 
                                                                                                                                left_on=echo_keys, 
                                                                                                                                right_on=bmg_keys)],
                                                                                                            ignore_index=True)
        self.echo_df = echo_bmg_linked_df.drop(columns=bmg_keys)
        return self
